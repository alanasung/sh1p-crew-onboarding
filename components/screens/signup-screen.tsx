...
export function SignupScreen({ initialData, onSubmit }: SignupScreenProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!trimmedLinkedIn) {
      ...
      newErrors.linkedIn = 'Enter a full LinkedIn profile URL (e.g. https://linkedin.com/in/yourname)'
    }
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-b from-navy via-[#0d1e33] to-navy flex items-center justify-center p-4 overflow-y-auto">
      ...
      <div className="absolute inset-0 flex items-center justify-center opacity-[0.08] blur-xl pointer-events-none overflow-hidden">
        <svg
          width="600"
          height="600"
          viewBox="0 0 320 320"
      ...
      {/* Parchment card */}
      <div className="relative w-full max-w-xl parchment rounded-lg p-6 md:p-8 shadow-2xl noise">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-3 bg-navy/5 border-2 rounded-lg font-mono text-navy
                focus:outline-none focus:border-gold transition-colors
                ${errors.email ? 'border-red-500' : 'border-rope/30'}`}
          ...
          <button
            type="submit"
            className="w-full mt-4 md:mt-6 px-8 py-4 bg-navy text-gold font-mono font-bold text-lg rounded-lg 
